...
of jobs?\n\
[Type 'yes' or 'no']\n")
#Repeats question and requires input to continue the story forward.
         uncle_question= str(sys.stdin.readline().strip())
...
#This code section (Naming the dog) means the user MUST enter a valid name or program will not execute

   while not dog_name:
            sys.stdout.write("\
What does the dog's name collar say?\n")
#Player is able to name the dog, value is stored and then repeated through-out the game
            dog_name=str(sys.stdin.readline().strip())
   sys.stdout.write("\
...
#Creates variable called wrap_up_text, this variable is used when all branches have been completed to finish
#off and round out the story for the player.
   wrap_up_text=("\
...
\nout this morning did it? It does that for me too. Uncle hands you a large\
\n beef bone from the butchers and a crisp $50 note for completing your jobs.\n\
“I got" +dog_name+ "this for the help I knew you'd get..\n\
...
#hash below is code character counter for creator to follow guidelines on limits                      
################################################################################
#Exposition for the Chickens branch of the story.
                    sys.stdout.write("\
...
#inserts answer from the question in float format
                            sys.stdout.write("\
You fill the bucket with "+(str(chicken_feed_question))+" Scoops of feed. \n\
That looks perfect!\n")
...
\n"+dog_name+" looks up at you triumphantly and rolls onto it's belly for a rub.\n\
...
\nIn Numbers, How many bags of oats will you need to put in the feeding trough?\n")
...
\nHow many bags of oats (in numbers) will you need to put in the feeding trough?\n")
                             oats_user_input= float(sys.stdin.readline().strip())
                                
#code does not allow user invalid input
#validates the user response against the correct answer for the program.
...
“that looks like a yes to me!")
#If all branches not yet completed, program will cycle back and offer the
#list of options to the player.
                       else:
                            sys.stdout.write("\
\n[Choose an option from the list - eg type 'Horses'\n\
